...
export function mountSlots(mountEl, store) {
  ...
  function clearHighlights(mountEl) {
    mountEl
      .querySelectorAll(".cell.winCell")
      .forEach((x) => x.classList.remove("winCell"));
    mountEl
      .querySelectorAll(".cell.scatterCell")
      .forEach((x) => x.classList.remove("scatterCell"));
  ...
  function renderWinBreakdown(el, theme, res, bet) {
    ...
    if (res.triggers?.freeSpins)
      parts.push(`Free Spins x${res.triggers.freeSpinsAward}`);
  ...
  function renderRules(el, theme) {
    ...
    const coinSyms = Object.keys(theme.symbols).filter(
      (s) => theme.symbols[s]?.coin || theme.symbols[s]?.jackpot,
    );
    ...
    <div style="display:grid; gap:10px;">
      <div>
        ...
        ${
          Object.keys(fsAwards).length
            ? `Awards: ${Object.entries(fsAwards)
                .map(([k, v]) => `${k}=${v}`)
                .join(", ")}.`
            : ``
        }
    ...
    for (const w of res.lineWins) {
      ...
      for (let reel = 0; reel < w.count; reel++) {
        ...
        const cell = mountEl.querySelector(
          `.cell[data-row="${row}"][data-reel="${reel}"]`,
        );
      ...
      const anchor = mountEl.querySelector(
        `.cell[data-row="${lastRow}"][data-reel="${lastReel}"]`,
      );
      if (anchor) {
  ...
  el.spin.addEventListener("click", async () => {
    ...
    try {
      ...
      const progCfg = theme.bonus?.holdSpin?.progressive;
      let jp = progCfg
        ? await loadProgressives(store, themeKey, progCfg.seed)
        : null;
      ...
      if (jp && freeSpinsLeft === 0) {
        ...
        jp.MINOR = Math.round(
          jp.MINOR + effectiveTotalBet * progCfg.rate.MINOR,
        );
        jp.MAJOR = Math.round(
          jp.MAJOR + effectiveTotalBet * progCfg.rate.MAJOR,
        );
      ...
      if (freeSpinsLeft === 0) {
        await store.placeBet(
          round.id,
          `BET:${useBet.linesEnabled}L@${useBet.betPerLine}`,
          effectiveTotalBet,
        );
      } else {
      ...
      // UI result
      const bonusText = res.triggers?.holdSpin
        ? "Hold&Spin"
        : res.triggers?.pickBonus
          ? "Pick Bonus"
          : "—";
...
async function runHoldSpinBonus(
  el,
  store,
  themeKey,
  theme,
...
function renderStatic(reels, theme) {
  reels.forEach((reel, i) => {
    ...
    strip.innerHTML = makeStripHTML(
      ["A", "K", "Q", "J", "T", "S", "B", "W", "A", "K", "Q"],
      theme,
      i,
    );
    ...
    (reel, r) =>
      new Promise((resolve) => {
        setTimeout(() => {
          ...
          const filler = [];
          const pool = Object.keys(theme.symbols).filter(
            (s) => !theme.symbols[s]?.jackpot && !theme.symbols[s]?.bonus,
          );
...
async function runPickBonus(el, store) {
  ...
  const render = () => {
    ...
    showOverlay(
      el,
      "Pick Bonus",
      `Picks left: <b>${remaining}</b> &nbsp; Total: <b>${total}</b><div class="pickGrid">${grid}</div>`,
      [
        ...(remaining === 0
          ? [
              {
