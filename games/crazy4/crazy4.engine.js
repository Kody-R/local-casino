...
export async function dealCrazy4Round(
  store,
  { ante, superBonus, queensUp, payouts },
) {
...
export async function playCrazy4(store, roundId, live) {
  ...
  if (live.superBonus > 0) {
    ...
    if (mult > 0)
      await store.settle(
        ...
        "SUPER_BONUS",
        "WIN",
        mult * live.superBonus,
        live.superBonus,
      );
    else await store.settle(roundId, "SUPER_BONUS", "LOSE", 0, 0);
  }
  ...
  if (live.queensUp > 0) {
    ...
    else await store.settle(roundId, "QUEENS_UP", "LOSE", 0, 0);
  }
