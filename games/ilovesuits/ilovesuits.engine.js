...
function sortDesc(vals) {
  return vals.slice().sort((a, b) => b - a);
}
...
export async function dealILSRound(
  store,
  { ante, flushRush, superFlushRush, payouts },
) {
  ...
  if (superFlushRush > 0)
    await store.placeBet(round.id, "SUPER_FLUSH_RUSH", superFlushRush);
...
export async function foldILS(store, roundId, live) {
  ...
  if (live.flushRush > 0)
    await store.settle(roundId, "FLUSH_RUSH", "LOSE", 0, 0);
  if (live.superFlushRush > 0)
    await store.settle(roundId, "SUPER_FLUSH_RUSH", "LOSE", 0, 0);
...
export async function playILS(store, roundId, live, mult) {
  ...
  if (!allowed.includes(mult))
    throw new Error("Invalid Play multiplier for your flush.");
  ...
  if (!live.dealerQualifies) {
    ...
    await store.closeRound(roundId);
    return {
      title: "DEALER NO QUALIFY",
      detail: "Ante wins 1:1; Play returned.",
    };
  }
