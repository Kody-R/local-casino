...
import {
  ...
  doubleDown,
  handTotals,
} from "./blackjackplus.engine.js";
import {
  loadBJPayouts,
  saveBJPayouts,
  payoutsTemplateHTML,
} from "./blackjackplus.payouts.js";
...
export function mountBlackjackPlus(mountEl, store) {
  ...
  el.deal.addEventListener("click", async () => {
    try {
      ...
      const { readBJPayoutsFromUI } =
        await import("./blackjackplus.payouts.js");
      const payouts = readBJPayoutsFromUI(mountEl);
      await store.setSetting("payouts_blackjackplus", payouts);

      const { roundId, live } = await dealBJRound(store, {
        wager,
        side3,
        payouts,
      });
