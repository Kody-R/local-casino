...
export function mountBonusBowling(rootEl, store) {
  ...
  <div id="bbLane" class="bb-lane">
    ...
    <div id="bbPins" class="bb-pins">
      ${Array.from({length:10}).map((_,i)=>`<div class="bb-pin" data-pin="${i}"></div>`).join("")}
    </div>
  </div>
  ...
  ${state.sessionLog.slice(-10).map(x => {
  ...
  function renderResult(s) {
    const r = s.roll;
    const kind = r.kind === "STRIKE" ? "STRIKE" : r.kind === "SPARE" ? "SPARE" : `OPEN (${r.total})`;
  ...
  state = setSession(state, { enabled: e.target.checked, frames: state.sessionFrames });
  paint();
});

rootEl.querySelector("#bbSessionFrames")?.addEventListener("input", (e) => {
  state = setSession(state, { enabled: state.sessionMode, frames: e.target.value });
});

rootEl.querySelector("#bbStartSession")?.addEventListener("click", async () => {
  ...
  await paint();
    ...
    rootEl.querySelector("#btnRoll")?.addEventListener("click", async () => {
      ...
      for (const t of BET_TYPES) {
        const v = Math.max(0, Math.floor(Number(rootEl.querySelector(`input[data-bet="${t}"]`)?.value || 0)));
        bets[t] = v;
      }
      ...
      if (state.goldenBonus > 0) {
        await store.settle(round.id, "GOLDEN_BONUS", "STRIKE_GOLDEN_BONUS", state.goldenBonus, 0);
      }

      await store.closeRound(round.id);
      await store.uiRefresh();
      await paint();
      requestAnimationFrame(() => runBowlingAnimation(state.roll, state.isGolden));
    });
  ...
  pinsEl.querySelectorAll(".bb-pin").forEach(p => p.classList.remove("down"));
}
