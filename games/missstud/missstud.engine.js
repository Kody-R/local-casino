...
export function canBetStreet(state) {
  ...
  if (state.phase === "STREET3" || state.phase === "STREET4" || state.phase === "STREET5") {
...
export function placeStreetBet(state, street, mult) {
  const m = Number(mult);
  if (![1,2,3].includes(m)) throw new Error("Street bet must be 1x, 2x, or 3x ante.");
...
export function fold(state) {
  ...
  state.result = { outcome: "FOLD", payoutMain: 0, payoutBonus: 0, detail: "Folded. All bets lose." };
  return state;
}
...
export function settle(state, paytable, bonus3table) {
  ...
  const totalMainBets = state.ante + state.street3 + state.street4 + state.street5;

  // “It pays on all bets” (main paytable multiplier applies to ante + all street bets). 
  const payoutMain = main.type === "LOSE"
    ? 0
    : (main.mult * totalMainBets);
...
function scoreMain(best, paytable) {
  ...
  if (k === "PAIR_6_TO_10") return { type:"PUSH", mult:0, label:"Pair (6–10) Push" };
  if (k === "PAIR_JACKS_PLUS") return { type:"WIN", mult:1, label:"Pair (Jacks+) 1:1" };

  const mult = paytable[k] ?? 0;
  if (mult > 0) return { type:"WIN", mult, label:`${best.handName} ${mult}:1` };
...
function evaluateMissStud5(cards5) {
  ...
  const isRoyal = isFlush && isStraight && uniq.includes(14) && uniq.includes(13) && uniq.includes(12) && uniq.includes(11) && uniq.includes(10);

  let rankKey = "OTHER";
  let handName = "High Card";

  if (isFlush && isStraight && isRoyal) { rankKey = "ROYAL_FLUSH"; handName = "Royal Flush"; }
  else if (isFlush && isStraight) { rankKey = "STRAIGHT_FLUSH"; handName = "Straight Flush"; }
  else if (has4) { rankKey = "FOUR_KIND"; handName = "Four of a Kind"; }
  else if (has3 && freq[1] === 2) { rankKey = "FULL_HOUSE"; handName = "Full House"; }
  else if (isFlush) { rankKey = "FLUSH"; handName = "Flush"; }
  else if (isStraight) { rankKey = "STRAIGHT"; handName = "Straight"; }
  else if (has3) { rankKey = "THREE_KIND"; handName = "Three of a Kind"; }
  else if (pairCount === 2) { rankKey = "TWO_PAIR"; handName = "Two Pair"; }
  else if (has2) {
    const pairRank = Number(Object.keys(counts).find(k => counts[k] === 2));
    if (pairRank >= 6 && pairRank <= 10) { rankKey = "PAIR_6_TO_10"; handName = "Pair (6–10)"; }
    else if (pairRank === 11 || pairRank === 12 || pairRank === 13 || pairRank === 14) {
      rankKey = "PAIR_JACKS_PLUS"; handName = "Pair (Jacks+)";
    } else {
      rankKey = "OTHER"; handName = "Low Pair";
    }
  }
